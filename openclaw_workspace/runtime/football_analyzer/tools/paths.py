...
def data_dir(*parts: str) -> str:
    ...
    if not _is_within(base_path, root):
        raise ValueError(f"OPENCLAW_FOOTBALL_DATA_DIR must be inside {root}")
    path = base_path.joinpath(*parts) if parts else base_path
    return str(path)


def datasets_dir(*parts: str) -> str:
    base = os.getenv("OPENCLAW_FOOTBALL_DATASETS_DIR")
    base_path = Path(base).expanduser() if base else _default_datasets_dir()
    base_path = base_path.resolve()
    root = _workspace_root().resolve()
    if not _is_within(base_path, root):
        raise ValueError(f"OPENCLAW_FOOTBALL_DATASETS_DIR must be inside {root}")
    path = base_path.joinpath(*parts) if parts else base_path
    return str(path)
