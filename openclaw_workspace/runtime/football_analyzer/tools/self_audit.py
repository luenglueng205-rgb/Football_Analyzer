...
def _detect_duplicate_modules(*, repo_root: Optional[Path]) -> Dict[str, Any]:
    ...
    for rel_dir in ["agents", "tools", "core", "scripts"]:
        a_dir = openclaw_root / rel_dir
        b_dir = standalone_root / rel_dir
                ...
                samples.append(
                    {
                        ...
                        "openclaw": str(a_files[rel]),
                        "standalone": str(b_files[rel]),
                    }
                )
...
def _generate_drift_bloat_diagnosis(*, ws_root: Path, repo_root: Optional[Path], mismatch: Dict[str, Any]) -> Dict[str, Any]:
    ...
    keep = [
        ...
        {
            "id": "self_audit_tool_entrypoint",
            "title": "保留 runtime 自检工具入口（MCP 可调用）",
            "files": [
                _safe_rel((ws_root / "tools" / "self_audit.py").resolve(), repo_root),
                _safe_rel((repo_root / "openclaw_workspace" / "src" / "mcp_server.py").resolve(), repo_root) if repo_root else "openclaw_workspace/src/mcp_server.py",
            ],
        },
    ]
    ...
    cut = [
        ...
        },
        {
            ...
            "title": "切断对 repo_root/data 的隐式依赖（只认 OPENCLAW_FOOTBALL_DATA_DIR）",
            "files": [
                "data/",
                "openclaw_workspace/runtime/football_analyzer/tools/paths.py",
            ],
        },
    ]
    ...
    slimming_plan = [
        {
            ...
            "actions": [
                {"id": "freeze_data_dir", "change": "统一 OPENCLAW_FOOTBALL_DATA_DIR，禁止隐式读取 repo_root/data", "evidence": "historical_data.root_vs_standalone"},
...
def collect_self_audit(
    *,
    ...
    data_dir_info = _resolve_data_dir(ws_root, "OPENCLAW_FOOTBALL_DATA_DIR")
        ...
        suggestions.append(
            {
                ...
                "title": "将根目录 data/chroma_db 迁移到 OPENCLAW_FOOTBALL_DATA_DIR/chroma_db 或重建",
        ...
        suggestions.append(
            {
                ...
                "title": "将根目录 data/raw 数据包迁移到 openclaw_workspace/data/raw 或调整引用路径",
        ...
        suggestions.append(
            {
                ...
                "title": "确保数据包位于 OPENCLAW_FOOTBALL_DATA_DIR/raw 下（推荐复制/rsync，而不是引用 root/data）",
...
def self_audit(*, online: bool = False, deep: bool = False) -> Dict[str, Any]:
    return collect_self_audit(offline=not bool(online), deep=bool(deep))
