...
def _apply_beidan_adjustment(schema: RecommendationSchema, *, ev_multiplier: float = 0.65) -> RecommendationSchema:
    adjusted: List[RecommendedBet] = []
    for b in schema.recommended_bets:
        ev = b.ev
        if isinstance(ev, (int, float)):
            ev = float(ev) * float(ev_multiplier)
        adjusted.append(
            RecommendedBet(
                match_id=b.match_id,
                lottery_type="BEIDAN",
                play_type="BEIDAN_WDL",
                market=str(b.market or "WDL"),
                selection=str(b.selection),
                prob=float(b.prob) if isinstance(b.prob, (int, float)) else None,
                odds=float(b.odds) if isinstance(b.odds, (int, float)) else None,
                ev=float(ev) if isinstance(ev, (int, float)) else None,
                edge=float(b.edge) if isinstance(b.edge, (int, float)) else None,
                risk_tags=list(b.risk_tags) if isinstance(b.risk_tags, list) else [],
            )
        )
    return RecommendationSchema(
        recommended_leagues=list(schema.recommended_leagues),
        recommended_bets=adjusted,
        recommended_parlays=list(schema.recommended_parlays),
        live_check=schema.live_check,
        post_match_review=schema.post_match_review,
        audit=schema.audit,
    )


class BeidanWorkflow:
    def __init__(
        self,
        *,
        ...
        if post_match_memory_hook is not None:
            ...
            def _default_post_hook(payload: Dict[str, Any]) -> Dict[str, Any]:
                ...
                tags = ["beidan_workflow", "post_match"]
    ...
    def run(
        ...
        *,
        ...
        if odds_norm.get("ok") is True and complete_odds:
            ...
            if base_schema.recommended_bets and str(base_schema.recommended_bets[0].lottery_type or "").upper() != "BEIDAN":
                base_schema = _apply_beidan_adjustment(base_schema, ev_multiplier=0.65)
        else:
        ...
        try:
            ...
            tags = ["beidan_workflow", "pre_match", f"league:{league_code}", f"action:{action}"]
            if primary:
        ...
        report = self.daily_reporter.generate_report(date_str=date, pnl=pnl, evolution_reason="beidan_workflow_demo")
