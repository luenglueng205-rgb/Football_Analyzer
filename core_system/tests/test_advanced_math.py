...
def test_calculate_beidan_sxds_matrix():
    ...
    result = AdvancedLotteryMath.calculate_beidan_sxds_matrix(matrix)
...
def test_optimize_jingcai_ticket():
    ...
    result = AdvancedLotteryMath.optimize_jingcai_ticket(num_legs=4, combined_odds=6000.0, target_investment=10000)
    ...
    assert result["max_bets_per_ticket"] == 50
...
def test_calculate_zucai_value_index():
    ...
    result = AdvancedLotteryMath.calculate_zucai_value_index(matches)
...
def test_map_poisson_to_jingcai_scores():
    ...
    matrix[5][0] = 0.05
    matrix[4][3] = 0.05
    result = AdvancedLotteryMath.map_poisson_to_jingcai_scores(matrix)
...
def test_calculate_parlay_kelly():
    ...
    result = AdvancedLotteryMath.calculate_parlay_kelly(legs)
    assert "ev" in result
...
def test_calculate_last_leg_hedge():
    result = AdvancedLotteryMath.calculate_last_leg_hedge(
        original_bet=100,
...
def test_beidan_parlay_kelly_bug_fix():
    ...
    result_jc = AdvancedLotteryMath.calculate_parlay_kelly(legs)
    assert result_jc["combined_odds"] == 8.0
    
    # Beidan mode
    result_bd = AdvancedLotteryMath.calculate_parlay_kelly(legs, lottery_type="BEIDAN")
